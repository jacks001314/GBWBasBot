...
func registerBase64Encoder(L *lua.LState) {
	...
	L.SetField(mt, "__index", luahelper.WriterFuncTable(L))
}
...
func registerBase64Decoder(L *lua.LState) {
	...
	L.SetField(mt, "__index", luahelper.ReaderFuncTable(L))
}
...
func NewEncoder(L *lua.LState) int {
	encoding := CheckBase64Encoding(L, 1)
	writer := luahelper.CheckIOWriter(L, 2)
	L.Pop(L.GetTop())
...
func NewDecoder(L *lua.LState) int {
	encoding := CheckBase64Encoding(L, 1)
	reader := luahelper.CheckIOReader(L, 2)
	L.Pop(L.GetTop())
